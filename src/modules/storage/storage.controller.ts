   ...
   public async giveDrugsByDoses(@Headers() headers, @Param() params) {
      try {
         ...
         const result2 = Object.keys(result).map((key) => [
            { dose: result[key].dose, drugId: key },
         ])
         ...
         for (let i = 0; i < arrayOfDrugIdsAndDoses.length; i++) {
            await this.drugService
               .findById(arrayOfDrugIdsAndDoses[i].drugId)
               .then((res) => {
                  if (
                     !res ||
                     res.storageId !== storage._id ||
                     res.count * res.weight < arrayOfDrugIdsAndDoses[i].dose
                  ) {
         ...
         for (let i = 0; i < arrayOfDrugIdsAndDoses.length; i++) {
            await this.drugService
               .findById(arrayOfDrugIdsAndDoses[i].drugId)
               .then(async (res) => {
                  await this.drugService.findByIdAndUpdate(res._id, {
                     count:
                        res.count - arrayOfDrugIdsAndDoses[i].dose / res.weight,
                  })
               })
         }
         storage.filled -= sum
