...
import { getRandomPatientIndicators, getRegisterDate } from '../../helper/functions';
...
export class PatientController {
  ...
  public async updatePatientStatistic(@Headers() headers) {
    try {
      ...
      const newPatients = await this.patientService.getPatientsByDoctorId(doctor._id);
  ...
  @Put('/:userId')
  public async changeUserProfileByDoctor(@Headers() headers, @Param() params, @Body() body: UpdateUserProfileDto) {
    try {
      const { userId } = params;
      const decodeToken = this.authService.decodeToken(headers.token);
      if (!decodeToken) {
