...
export class DoctorController {
  ...
  @Put()
  public async updateDoctorProfile(@Headers() headers, @Body() body: UpdateDoctorDto) {
    try {
      const decodeToken = this.authService.decodeToken(headers.token);
      const { role, id } = decodeToken;
      if (role !== RolesEnum.doctor) {
      ...
      await this.doctorService.findDoctorByIdAndUpdate(id, body);
  ...
  @Post()
  public async createPatient(@Body() body) {
    try {
