...
export class AuthController {
  ...
  @Post('/register')
  async registerDoctor(@Body() body: createDoctorBodyDto): Promise<DoctorRegisterOrLoginResponseDto> {
    try {
      const { name, surname, email, password, bornDate, img, description } = body;
      if (!email.includes('@') || !email.includes('.') || email.includes('@.')) {
        return { error: ErrorsEnum.incorrectEmail };
      }
      const candidateDoctor = await this.doctorService.findDoctorByEmail(email);
      if (candidateDoctor) {
        return { error: ErrorsEnum.exist };
      }
      const candidatePatient = await this.patientService.findPatientByEmail(email);
      if (candidatePatient) {
        return { error: ErrorsEnum.exist };
      }
      ...
      return {
        token: this.authService.accessToken(doctor._id, name, surname, doctor.role),
      };
    } catch (e) {
  ...
  @Post('/login')
  public async loginDoctor(@Body() body: LoginDoctorDto): Promise<DoctorRegisterOrLoginResponseDto> {
    try {
      ...
      const isMatch = this.authService.bcryptPassword(password, doctor.password);
      if (!isMatch) {
        return { error: ErrorsEnum.incorrectPassword };
      }
      return {
        token: this.authService.accessToken(doctor._id, doctor.name, doctor.surname, doctor.role),
      };
    } catch (e) {
      return { error: e.message };
    }
